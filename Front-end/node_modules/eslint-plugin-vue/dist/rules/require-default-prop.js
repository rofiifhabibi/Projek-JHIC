...
var require_require_default_prop = /* @__PURE__ */ require_runtime.__commonJSMin(((exports, module) => {
	...
	*/
	...
	const { inferRuntimeType } = require_ts_ast$1.default;
		...
		create(context) {
			...
			*/
			function isBooleanModel(model, options) {
				if (options) {
					if (isValueNodeOfBooleanType(options)) return true;
					if (options.type === "ObjectExpression" && options.properties.some((property) => property.type === "Property" && utils.getStaticPropertyName(property) === "type" && isValueNodeOfBooleanType(property.value))) return true;
				}
				if (model.typeNode) {
			...
			*/
			...
			return utils.compositingVisitors(utils.defineScriptSetupVisitor(context, {
				...
				onDefineModelEnter(node, model) {
					...
					if (isBooleanModel(model, options)) return;
					const objectOptions = options?.type === "ObjectExpression" ? options : null;
					if (objectOptions && (propIsRequired(objectOptions) || propHasDefault(objectOptions))) return;
					context.report({
