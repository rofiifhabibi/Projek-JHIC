...
// src/rules/no-action-timeout.ts
var actionMethods = /* @__PURE__ */ new Set([
  "blur",
  "check",
  "clear",
  "click",
...
function findTimeoutOption(node) {
...
var no_action_timeout_default = createRule({
  create(context) {
    const allowed = new Set(context.options[0]?.allow ?? []);
    return {
      MemberExpression(node) {
        const method = getStringValue(node.property);
        if (!actionMethods.has(method) || allowed.has(method) || node.parent.type !== "CallExpression" || node.parent.callee !== node) {
          return;
        }
  ...
  meta: {
    ...
    messages: {
      noActionTimeout: "Unexpected `timeout` option on `{{ method }}()`. Configure timeouts globally instead."
    },
    schema: [
      {
        additionalProperties: false,
...
var no_export_default = createRule({
  create(context) {
    const exportNodes = [];
    ...
    return {
      "AssignmentExpression > MemberExpression"(node) {
        ...
        if (!isIdentifier(object, "module") || isDeclaredInScope(context.sourceCode.getScope(object), "module")) {
          return;
        }
      ...
      "ExportAllDeclaration, ExportDefaultDeclaration, ExportNamedDeclaration, TSExportAssignment"(node) {
        exportNodes.push(node);
      },
      "Program:exit"() {
        if (!hasTestCase) {
...
var newDescribeContext = () => ({
  describeTitles: [],
  testTitles: []
});
var no_identical_title_default = createRule({
  create(context) {
    ...
    return {
      "CallExpression"(node) {
        ...
        const titles = call.type === "describe" ? currentLayer.describeTitles : currentLayer.testTitles;
...
// src/rules/no-magic-timeouts.ts
function evaluate(node) {
  ...
  if (node.type === "UnaryExpression" && (node.operator === "-" || node.operator === "+")) {
    ...
    return value === void 0 ? void 0 : node.operator === "-" ? -value : value;
  }
  if (node.type === "BinaryExpression") {
    const left = node.left.type === "PrivateIdentifier" ? void 0 : evaluate(node.left);
...
function isInsideConfig(node) {
  let current = node;
  while (current) {
    if (current.type === "CallExpression" && getStringValue(
...
function isCallArgument(object) {
  ...
  return (parent?.type === "CallExpression" || parent?.type === "NewExpression") && parent.arguments.includes(object);
}
var no_magic_timeouts_default = createRule({
  create(context) {
    const {
      allow = [],
      minOccurrences = 1,
    ...
    function collect(node) {
      ...
      if (value === void 0 || allowed.has(value)) {
        return;
      }
    ...
    return {
      "CallExpression"(node) {
        if (node.callee.type === "MemberExpression" && getStringValue(node.callee.property) === "setTimeout" && node.arguments.length === 1 && node.arguments[0].type !== "SpreadElement") {
  ...
  meta: {
    ...
    messages: {
      noMagicTimeout: "Avoid the magic timeout `{{ value }}`. Extract it to a named constant, or configure it globally in your Playwright config.",
      repeatedMagicTimeout: "The magic timeout `{{ value }}` is used {{ count }} times in this file. Extract it to a named constant, or configure it globally in your Playwright config."
    },
    schema: [
      {
        additionalProperties: false,
...
function isTestInfo(context, node) {
  if (node.type !== "Identifier") {
    return false;
  }
...
var no_skipped_test_default = createRule({
  create(context) {
    ...
    const allowConditional = typeof options.allowConditional === "object" ? {
    ...
    const isSkipAnnotation = (value) => value === "skip" || disallowFixme && value === "fixme";
    return {
      CallExpression(node) {
        ...
        } else if (node.callee.type === "MemberExpression" && isSkipAnnotation(getStringValue(node.callee.property)) && isTestInfo(context, node.callee.object)) {
          isStandalone = true;
        ...
        if (isStandalone && allowConditional[annotation] && (node.arguments.length !== 0 || findParent(node, "BlockStatement")?.parent?.type === "IfStatement" || findParent(node, "SwitchCase") !== void 0)) {
          return;
        }
        context.report({
          data: { annotation },
...
// src/rules/no-template-literal-title.ts
var methods3 = {
...
var no_template_literal_title_default = createRule({
  create(context) {
    const { ignore } = {
      ignore: [],
      ...context.options?.[0] ?? {}
    ...
    return {
      CallExpression(node) {
        ...
        const method = methods3[call.type];
        ...
        context.report({
          data: { method },
          fix: fixable ? (fixer) => fixer.replaceText(title, quote(cooked)) : void 0,
  ...
  meta: {
    ...
    fixable: "code",
    messages: {
      interpolatedTitle: "Do not interpolate values into `{{method}}` titles. Titles are used by reporters and `--grep`, so they should be stable.",
      staticTemplateLiteral: "Use a string literal instead of a template literal for `{{method}}` titles."
    },
    schema: [
      {
        additionalProperties: false,
...
var no_test_return_statement_default = createRule({
  create(context) {
    return {
      CallExpression(node) {
        if (!isTypeOfFnCall(context, node, ["test"])) {
...
var locatorMethods3 = /* @__PURE__ */ new Set([
  "and",
  "first",
  "getByAltText",
  "getByLabel",
...
function isSupportedMethod(node) {
  ...
  return locatorMethods3.has(name) || pageMethods2.has(name) && isPageMethod(node, name);
}
...
function getLastStatement(fn) {
  ...
  return lastStatement?.type === "ExpressionStatement" ? lastStatement.expression : lastStatement?.type === "ReturnStatement" ? lastStatement.argument ?? void 0 : lastStatement;
}
var prefer_ending_with_an_expect_default = createRule({
  create(context) {
    const options = {
      assertFunctionNames: [],
      assertFunctionPatterns: [],
    ...
    function isAssertion(node) {
      ...
      if (call?.type === "step") {
        const body = getBody2(node);
        return !!body && isAssertion(getLastStatement(body));
      }
      return options.assertFunctionNames.some((name) => dig(node.callee, name)) || patterns.some((pattern) => dig(node.callee, pattern));
    }
    return {
      CallExpression(node) {
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        const methodConfig = methods4[method];
        if (!Object.hasOwn(methods4, method)) {
          return;
        }
...
// src/rules/require-annotation-reason.ts
var DEFAULT_ANNOTATIONS = ["fixme", "skip"];
var require_annotation_reason_default = createRule({
  create(context) {
    const { annotations = DEFAULT_ANNOTATIONS, pattern } = context.options?.[0] ?? {};
    const names = new Set(annotations);
    const regex = pattern ? new RegExp(pattern, "u") : void 0;
    return {
      CallExpression(node) {
        const call = parseFnCall(context, node);
        if (call?.type !== "config") {
  ...
  meta: {
    ...
    messages: {
      missingReason: "The `.{{ annotation }}()` annotation should say why. Add a reason as the last argument.",
...
var plugin = {
  ...
  rules: {
    ...
    "no-action-timeout": no_action_timeout_default,
    ...
    "no-magic-timeouts": no_magic_timeouts_default,
    ...
    "no-template-literal-title": no_template_literal_title_default,
    ...
    "prefer-ending-with-an-expect": prefer_ending_with_an_expect_default,
    ...
    "require-annotation-reason": require_annotation_reason_default,
