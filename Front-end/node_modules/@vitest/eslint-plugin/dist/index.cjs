...
const findVitestModeProperty = (node, mode) => {
	...
	if (options?.type !== _typescript_eslint_utils.AST_NODE_TYPES.ObjectExpression) return null;
	const property = options.properties.findLast((property) => property.type === _typescript_eslint_utils.AST_NODE_TYPES.Property && isSupportedAccessor(property.key, mode));
	if (property?.value.type !== _typescript_eslint_utils.AST_NODE_TYPES.Literal || property.value.value !== true) return null;
	return property;
};
/**
...
const getCallExpression = (member) => member.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression && member.parent.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.CallExpression && member.parent.parent.callee === member.parent ? member.parent.parent : null;
...
const parseExpectCallExpression = (node, topMostCallExpression, typeLessParsedVitestFnCall) => {
	const chains = typeLessParsedVitestFnCall.members.map((member, index) => classifyExpectChain(member, index === 0 && typeLessParsedVitestFnCall.head.node.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression));
	const matcherIndex = chains.findIndex((chain) => chain.kind === "matcher");
	if (node.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression && topMostCallExpression !== node) return null;
	...
	return {
		...
		expectCall: expectApi?.callExpression ?? (typeLessParsedVitestFnCall.head.node.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.CallExpression ? typeLessParsedVitestFnCall.head.node.parent : null),
...
const parseExpectTypeOfCallExpression = (node, topMostCallExpression, typeLessParsedVitestFnCall) => {
	...
	return {
		...
		matcher,
		expectApiName: null,
		expectCall: typeLessParsedVitestFnCall.head.node.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.CallExpression ? typeLessParsedVitestFnCall.head.node.parent : null,
	...
	create(context) {
		...
		return {
			ImportDeclaration(node) {
				...
				if (node.source.type !== _typescript_eslint_utils.AST_NODE_TYPES.Literal || node.source.value !== "vitest") {
					for (const spec of node.specifiers) if (spec.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportSpecifier && spec.local.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier && vitestNamespaceNames.has(spec.local.name)) namespaceLocalNames.delete(spec.local.name);
					return;
				}
				for (const spec of node.specifiers) if (spec.type === _typescript_eslint_utils.AST_NODE_TYPES.ImportSpecifier && spec.imported.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier && vitestNamespaceNames.has(spec.imported.name) && spec.local.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) namespaceLocalNames.add(spec.local.name);
			},
			CallExpression(node) {
				if (node.callee.type !== _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression) return;
				const { object, property } = node.callee;
				if (object.type !== _typescript_eslint_utils.AST_NODE_TYPES.Identifier || !namespaceLocalNames.has(object.name) || property.type !== _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return;
	...
	create(context, [{ additionalTestBlockFunctions = [] }]) {
		...
		return {
			...
			"CallExpression:exit"(node) {
				...
				if (top === "test" && (isTypeOfVitestFnCall(node, context, ["test"]) || isCustomTestBlockFunction(node)) && node.callee.type !== _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression || top === "helper" && isDefineHelperCall(node) || top === "template" && node.callee.type === _typescript_eslint_utils.AST_NODE_TYPES.TaggedTemplateExpression) callStack.pop();
			},
...
var prefer_expect_type_of_default = createEslintRule({
	...
	create(context) {
		return { CallExpression(node) {
			...
			if (!matcherArg || matcherArg.type === _typescript_eslint_utils.AST_NODE_TYPES.SpreadElement) return;
...
const findTopMostMatcherCallExpression = (node) => {
	...
	while (currentNode.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.MemberExpression && currentNode.parent.object === currentNode && isSupportedAccessor(currentNode.parent.property)) {
		if (promiseChainMethods.has(getAccessorValue(currentNode.parent.property))) break;
		currentNode = currentNode.parent;
		if (currentNode.parent?.type === _typescript_eslint_utils.AST_NODE_TYPES.CallExpression && currentNode.parent.callee === currentNode) {
		...
		*/
		...
		return {
			CallExpression(node) {
				...
				if (matcherCallExpression.type !== _typescript_eslint_utils.AST_NODE_TYPES.CallExpression) return;
...
var valid_title_default = createEslintRule({
	...
	create(context, [{ ignoreTypeOfDescribeName, allowArguments, disallowedWords = [], mustNotMatch, mustMatch }]) {
		...
		return { CallExpression(node) {
			...
			if (allowArguments && argument.type === _typescript_eslint_utils.AST_NODE_TYPES.Identifier) return;
