...
const findVitestModeProperty = (node, mode) => {
	const options = node.arguments[1];
	if (options?.type !== AST_NODE_TYPES.ObjectExpression) return null;
	const property = options.properties.findLast((property) => property.type === AST_NODE_TYPES.Property && isSupportedAccessor(property.key, mode));
	if (property?.value.type !== AST_NODE_TYPES.Literal || property.value.value !== true) return null;
	return property;
};
/**
...
let HookName = /* @__PURE__ */ function(HookName) {
	...
	HookName["aroundEach"] = "aroundEach";
	HookName["aroundAll"] = "aroundAll";
	return HookName;
}({});
...
const hasInvalidExpectChain = (chains, matcherIndex) => {
	const modifierChains = matcherIndex === -1 ? chains : chains.slice(0, matcherIndex);
	if (modifierChains.some((chain) => chain.kind === "unknown" || !["expect-api", "matcher"].includes(chain.kind) && chain.callExpression)) return true;
	const modifierNames = modifierChains.filter((chain) => chain.kind === "modifier").map((chain) => getAccessorValue(chain.member));
	if (modifierNames.filter((name) => name === ModifierName.not).length > 1) return true;
	if (modifierNames.filter((name) => promiseExpectModifiers.has(name)).length > 1) return true;
	return !chains.slice(matcherIndex + 1).every((chain) => chain.kind !== "unknown" && (chain.kind === "expect-api" || chain.kind === "matcher" || !chain.callExpression));
};
...
const expectModifiers = new Set([
	ModifierName.not,
	ModifierName.rejects,
	ModifierName.resolves
]);
const expectApiMethods = new Set([
	"element",
	"poll",
	"soft"
]);
const promiseExpectModifiers = new Set([ModifierName.rejects, ModifierName.resolves]);
const expectTypeOfModifiers = new Set([
	ModifierName.not,
	ModifierName.resolves,
...
const getCallExpression = (member) => member.parent?.type === AST_NODE_TYPES.MemberExpression && member.parent.parent?.type === AST_NODE_TYPES.CallExpression && member.parent.parent.callee === member.parent ? member.parent.parent : null;
const classifyExpectChain = (member, isFirstMemberOfStaticExpectCall = false) => {
	const name = getAccessorValue(member);
	const callExpression = getCallExpression(member);
	if (callExpression) {
		if (isFirstMemberOfStaticExpectCall && expectApiMethods.has(name)) return {
			kind: "expect-api",
			member,
			callExpression
		};
		if (isFirstMemberOfStaticExpectCall && name === "any") return {
			kind: "matcher",
			member,
			callExpression
		};
		if (expectModifiers.has(name) || chaiChainableProperties.has(name) && !chaiDualRoleChainProperties.has(name)) return {
			kind: "language-chain",
			member,
			callExpression
		};
	...
	if (isFirstMemberOfStaticExpectCall && name === "assert") return {
		kind: "expect-api",
		member,
		callExpression
	};
	if (chaiChainableProperties.has(name)) return {
		kind: "language-chain",
		member,
		callExpression
	};
	if (!expectModifiers.has(name)) return {
		kind: "unknown",
		member,
		callExpression
	};
...
const parseExpectCallExpression = (node, topMostCallExpression, typeLessParsedVitestFnCall) => {
	const chains = typeLessParsedVitestFnCall.members.map((member, index) => classifyExpectChain(member, index === 0 && typeLessParsedVitestFnCall.head.node.parent?.type === AST_NODE_TYPES.MemberExpression));
	const matcherIndex = chains.findIndex((chain) => chain.kind === "matcher");
	if (node.parent?.type === AST_NODE_TYPES.MemberExpression && topMostCallExpression !== node) return null;
	...
	return {
		...
		expectCall: expectApi?.callExpression ?? (typeLessParsedVitestFnCall.head.node.parent?.type === AST_NODE_TYPES.CallExpression ? typeLessParsedVitestFnCall.head.node.parent : null),
		args: matcher.callExpression?.arguments ?? [],
		modifiers: modifierChains.filter((chain) => chain.kind === "modifier").map((chain) => chain.member)
	};
};
const parseExpectTypeOfCallExpression = (node, topMostCallExpression, typeLessParsedVitestFnCall) => {
	const matcherIndex = typeLessParsedVitestFnCall.members.findIndex((member) => getCallExpression(member));
	const modifierMembers = matcherIndex === -1 ? typeLessParsedVitestFnCall.members : typeLessParsedVitestFnCall.members.slice(0, matcherIndex);
	const modifierNames = modifierMembers.map(getAccessorValue);
	if (node.parent?.type === AST_NODE_TYPES.MemberExpression && topMostCallExpression !== node) return null;
	if (modifierMembers.some((member) => !expectTypeOfModifiers.has(getAccessorValue(member))) || modifierNames.filter((name) => name === ModifierName.not).length > 1 || matcherIndex !== -1 && expectTypeOfModifiers.has(getAccessorValue(typeLessParsedVitestFnCall.members[matcherIndex]))) return "modifier-unknown";
	if (matcherIndex === -1) {
		if (node.parent?.type === AST_NODE_TYPES.MemberExpression) return "matcher-not-called";
		return "matcher-not-found";
	}
	...
	return {
		...
		matcher,
		expectApiName: null,
		expectCall: typeLessParsedVitestFnCall.head.node.parent?.type === AST_NODE_TYPES.CallExpression ? typeLessParsedVitestFnCall.head.node.parent : null,
...
const parseVitestFnCallWithReasonInner = (node, context) => {
	...
	if (type === "expect" || type === "expectTypeOf") {
		...
		return type === "expect" ? parseExpectCallExpression(node, topMostCallExpression, parsedVitestFnCall) : parseExpectTypeOfCallExpression(node, topMostCallExpression, parsedVitestFnCall);
	}
...
var hoisted_apis_on_top_default = createEslintRule({
	...
	create(context) {
		...
		const nodesToReport = [];
		const namespaceLocalNames = new Set(vitestNamespaceNames);
		return {
			ImportDeclaration(node) {
				lastImportEnd = node.range[1];
				if (node.source.type !== AST_NODE_TYPES.Literal || node.source.value !== "vitest") {
					for (const spec of node.specifiers) if (spec.type === AST_NODE_TYPES.ImportSpecifier && spec.local.type === AST_NODE_TYPES.Identifier && vitestNamespaceNames.has(spec.local.name)) namespaceLocalNames.delete(spec.local.name);
					return;
				}
				for (const spec of node.specifiers) if (spec.type === AST_NODE_TYPES.ImportSpecifier && spec.imported.type === AST_NODE_TYPES.Identifier && vitestNamespaceNames.has(spec.imported.name) && spec.local.type === AST_NODE_TYPES.Identifier) namespaceLocalNames.add(spec.local.name);
			},
			CallExpression(node) {
				if (node.callee.type !== AST_NODE_TYPES.MemberExpression) return;
				const { object, property } = node.callee;
				if (object.type !== AST_NODE_TYPES.Identifier || !namespaceLocalNames.has(object.name) || property.type !== AST_NODE_TYPES.Identifier) return;
	...
	create(context) {
		...
		return {
			CallExpression(node) {
				...
				const skipProperty = vitestFnCall.type === "describe" || vitestFnCall.type === "test" ? findVitestModeProperty(node, "skip") : null;
				const skipNode = skipMember ?? skipProperty?.key;
				if (vitestFnCall.name.startsWith("x") || skipNode) context.report({
...
var no_focused_tests_default = createEslintRule({
	...
	create: (context, options) => {
		...
		return { CallExpression(node) {
			...
			if (isTestOrDescribe && focusedNode) context.report({
				node: focusedNode,
				messageId: "noFocusedTests",
				fix: (fixer) => {
					if (!fixable) return null;
					if (onlyNode) return fixer.removeRange([onlyNode.range[0] - 1, onlyNode.range[1]]);
					if (onlyProperty) return fixer.replaceText(onlyProperty.value, "false");
					return null;
				}
			});
		} };
...
var no_standalone_expect_default = createEslintRule({
	...
	create(context, [{ additionalTestBlockFunctions = [] }]) {
		...
		const isDefineHelperCall = (node) => {
			const vitestFnCall = parseVitestFnCall(node, context);
			return vitestFnCall?.type === "vi" && vitestFnCall.members.length === 1 && getAccessorValue(vitestFnCall.members[0]) === "defineHelper";
		};
		return {
			CallExpression(node) {
				const vitestFnCall = parseVitestFnCall(node, context);
			...
			"CallExpression:exit"(node) {
				...
				if (top === "test" && (isTypeOfVitestFnCall(node, context, ["test"]) || isCustomTestBlockFunction(node)) && node.callee.type !== AST_NODE_TYPES.MemberExpression || top === "helper" && isDefineHelperCall(node) || top === "template" && node.callee.type === AST_NODE_TYPES.TaggedTemplateExpression) callStack.pop();
			},
...
var prefer_called_with_default = createEslintRule({
	...
	meta: {
		...
		messages: { preferCalledWith: "Prefer {{ matcherName }}(/* expected args */)" },
		type: "suggestion",
		fixable: "code",
		schema: []
	},
	create(context) {
		return { CallExpression(node) {
			...
			const preferredMatcher = PREFERRED_MATCHERS[matcherName];
			context.report({
				data: { matcherName: preferredMatcher },
				messageId: "preferCalledWith",
				node: matcher,
				fix: (fixer) => [replaceAccessorFixer(fixer, matcher, preferredMatcher)]
			});
		} };
	}
});
...
var prefer_expect_assertions_default = createEslintRule({
	...
	meta: {
		...
		messages: {
			...
			preferAssertionsOverHasAssertions: "Prefer `expect.assertions(<number of assertions>)` over `expect.hasAssertions()`",
			suggestAddingHasAssertions: "Add `expect.hasAssertions()`",
			suggestAddingAssertions: "Add `expect.assertions(<number of assertions>)`",
			suggestReplacingWithAssertions: "Replace with `expect.assertions(<number of assertions>)`",
	...
	create(context, [options]) {
		...
		function checkExpectHasAssertions(expectFnCall, func) {
			if (getAccessorValue(expectFnCall.members[0]) === "hasAssertions") {
				...
				else if (options.disallowHasAssertions) context.report({
					messageId: "preferAssertionsOverHasAssertions",
					node: expectFnCall.matcher,
					suggest: [{
						messageId: "suggestReplacingWithAssertions",
						fix: (fixer) => fixer.replaceText(expectFnCall.members[0], "assertions")
					}]
				});
				return;
			}
		...
		return {
			...
			"CallExpression:exit"(node) {
				...
				if (secondArg.body.type === AST_NODE_TYPES.BlockStatement) {
					...
					if (!options.disallowHasAssertions) suggestions.push(["suggestAddingHasAssertions", `${prefix}expect.hasAssertions();`]);
					suggestions.push(["suggestAddingAssertions", `${prefix}expect.assertions();`]);
				}
				context.report({
...
var prefer_expect_type_of_default = createEslintRule({
	...
	meta: {
		...
		schema: [],
		fixable: "code",
		messages: { preferExpectTypeOf: "Use `expect({{ value }}).toBeTypeOf({{ type }})` instead of `expect(typeof {{ value }}).toBe({{ type }})`" }
	},
	create(context) {
		return { CallExpression(node) {
			const vitestFnCall = parseVitestFnCall(node, context);
			...
			if (!matcherArg || matcherArg.type === AST_NODE_TYPES.SpreadElement) return;
			...
			context.report({
				...
				fix(fixer) {
					return fixer.replaceText(node, `expect(${valueText})${modifierPrefix}.toBeTypeOf(${typeText})`);
				}
			});
		} };
	}
...
var require_test_timeout_default = createEslintRule({
	...
	create(context) {
		function resolveConstTimeout(node, propName = "timeout") {
			...
			if (node.type === AST_NODE_TYPES.UnaryExpression && (node.operator === "-" || node.operator === "+") && node.argument.type === AST_NODE_TYPES.Literal && typeof node.argument.value === "number") return node.operator === "-" ? -node.argument.value : node.argument.value;
			if (node.type === AST_NODE_TYPES.ObjectExpression) {
				for (const prop of node.properties) {
					if (prop.type !== AST_NODE_TYPES.Property) continue;
					const key = prop.key;
					if (key.type === AST_NODE_TYPES.Identifier && key.name === propName || key.type === AST_NODE_TYPES.Literal && key.value === propName) {
						if (prop.value.type === AST_NODE_TYPES.Literal && typeof prop.value.value === "number") return prop.value.value;
			...
			if (node.type === AST_NODE_TYPES.Identifier) {
				...
				for (const def of variable.defs) {
					...
					if (init.type === AST_NODE_TYPES.Literal && typeof init.value === "number") return init.value;
					if (init.type === AST_NODE_TYPES.ObjectExpression) for (const p of init.properties) {
						if (p.type !== AST_NODE_TYPES.Property) continue;
						const key = p.key;
						if (key.type === AST_NODE_TYPES.Identifier && key.name === propName || key.type === AST_NODE_TYPES.Literal && key.value === propName) {
							if (p.value.type === AST_NODE_TYPES.Literal && typeof p.value.value === "number") return p.value.value;
				...
				if (firstMember && getAccessorValue(firstMember) === "setConfig") {
					...
					if (arg && arg.type === AST_NODE_TYPES.ObjectExpression) {
						for (const prop of arg.properties) if (prop.type === AST_NODE_TYPES.Property) {
							const key = prop.key;
							if (key.type === AST_NODE_TYPES.Identifier && key.name === "testTimeout" || key.type === AST_NODE_TYPES.Literal && key.value === "testTimeout") {
			...
			*/
			...
			for (const a of args) {
				...
				if (a.type === AST_NODE_TYPES.ObjectExpression) for (const prop of a.properties) {
					if (prop.type !== AST_NODE_TYPES.Property) continue;
					const key = prop.key;
					if (key.type === AST_NODE_TYPES.Identifier && key.name === "timeout" || key.type === AST_NODE_TYPES.Literal && key.value === "timeout") {
...
const promiseChainMethods = new Set([
	"catch",
	"finally",
	"then"
]);
const findTopMostMatcherCallExpression = (node) => {
	let currentNode = node;
	let topMostMatcherCallExpression = node;
	while (currentNode.parent?.type === AST_NODE_TYPES.MemberExpression && currentNode.parent.object === currentNode && isSupportedAccessor(currentNode.parent.property)) {
		if (promiseChainMethods.has(getAccessorValue(currentNode.parent.property))) break;
		currentNode = currentNode.parent;
		if (currentNode.parent?.type === AST_NODE_TYPES.CallExpression && currentNode.parent.callee === currentNode) {
		...
		return {
			CallExpression(node) {
				...
				if (expect.arguments.length < minArgs) {
					const expectLength = expect.callee.loc.end.column - expect.loc.start.column;
					const loc = {
						start: {
				...
				if (expect.arguments.length > maxArgs) {
					if (expect.arguments.length === 2) {
						...
						const takesOptionsArg = vitestFnCall.expectApiName === "poll" || vitestFnCall.expectApiName === "element";
						if (isSecondArgString || isSecondArgTemplateLiteral || takesOptionsArg) return;
					}
				...
				const parentNode = findTopMostMatcherCallExpression(matcherCallExpression);
				const shouldBeAwaited = vitestFnCall.modifiers.some((modifier) => {
					const value = getAccessorValue(modifier);
					return value === ModifierName.resolves || value === ModifierName.rejects;
		...
		return { CallExpression(node) {
			...
			const reportEmptyTitle = (node) => {
				context.report({
					...
					data: { functionName: vitestFnCall.type === "describe" ? DescribeAlias.describe : trimFXPrefix(vitestFnCall.name) === TestCaseName.it ? TestCaseName.it : TestCaseName.test },
					node
				});
			};
			const [argument] = node.arguments;
			...
			if (allowArguments && argument.type === AST_NODE_TYPES.Identifier) return;
