...
import type { ComponentInternalInstance, ComponentOptions, ComponentPublicInstance, ComputedOptions, CreateComponentPublicInstance, FunctionalComponent, MethodOptions } from 'vue';
import type { DomEventNameWithModifier } from './constants/dom-events';
import type { VueWrapper } from './vueWrapper';
import type { DefinedComponent, FindAllComponentsSelector, FindComponentSelector, NameSelector, RefSelector, VueNode } from './types';
...
export default abstract class BaseWrapper<ElementType extends Node> implements WrapperLike {
    ...
    findComponent<T extends (...args: any[]) => VNode>(selector: T): unknown extends ReturnType<T> ? DOMWrapper<Node> : '__ctx' extends keyof ReturnType<T> ? VueWrapper : DOMWrapper<Node>;
    ...
    findAllComponents<T extends (...args: any[]) => VNode>(selector: T): unknown extends ReturnType<T> ? DOMWrapper<Node>[] : '__ctx' extends keyof ReturnType<T> ? VueWrapper[] : DOMWrapper<Node>[];
    ...
    getComponent<T extends DefinedComponent>(selector: T | Exclude<FindComponentSelector, FunctionalComponent<any>>): Omit<VueWrapper<InstanceType<T>>, 'exists'>;
    getComponent<T extends (...args: any[]) => VNode>(selector: T): unknown extends ReturnType<T> ? Omit<DOMWrapper<Element>, 'exists'> : '__ctx' extends keyof ReturnType<T> ? Omit<VueWrapper, 'exists'> : Omit<DOMWrapper<Element>, 'exists'>;
    getComponent<T extends FunctionalComponent<any>>(selector: T | string): Omit<DOMWrapper<Element>, 'exists'>;
